...
public class MiscConfigManagerImpl extends AbstractBindingAwareConsumer implements
    ...
    public boolean disableStp(Long nodeId){
        ...
        if (config == null) {
            ...
            if (config == null) {
                logger.debug("ERROR: ConfigManager: disableStp(): MiscConfigService is null, nodeId = {}", nodeId);
                return false;
            }
        }
        ...
        try {
            ...
            if(result == null){
                logger.debug("ERROR: ConfigManager: disableStp(): call MiscConfigService.disableStp() fail (null result), nodeId = {}", nodeId);
                return false;
            }
            if(result.getResult() == null){
                logger.debug("ERROR: ConfigManager: disableStp(): call MiscConfigService.disableStp() fail (null in result), nodeId = {}", nodeId);
                return false;
            }
    ...
    public boolean enableStp(Long nodeId){
        ...
        if (config == null) {
            config = this.session.getRpcService(MiscConfigService.class);
            if (config == null) {
                logger.debug("ERROR: ConfigManager: enableStp(): MiscConfigService is null, nodeId = {}", nodeId);
                return false;
            }
        }
        ...
        try {
            ...
            if(result == null){
                logger.debug("ERROR: ConfigManager: enableStp(): call MiscConfigService.enableStp() fail (null result), nodeId = {}", nodeId);
                return false;
            }
            if(result.getResult() == null){
                logger.debug("ERROR: ConfigManager: enableStp(): call MiscConfigService.ensableStp() fail (null in result), nodeId = {}", nodeId);
                return false;
            }
    ...
    public boolean setStpPortState(Long nodeId, Short port, Boolean isEnable){
        ...
        if (config == null) {
            config = this.session.getRpcService(MiscConfigService.class);
            if (config == null) {
                logger.debug("ERROR: ConfigManager: setStpPortState(): MiscConfigService is null, nodeId = {}", nodeId);
                return false;
            }
        }
        ...
        try {
            ...
            if(result == null){
                logger.debug("ERROR: ConfigManager: setStpPortState(): call MiscConfigService.setStpPortState() fail (null result), nodeId {} port {} isEnable {}", nodeId, port, isEnable);
                return false;
            }
            if(result.getResult() == null){
                logger.debug("ERROR: ConfigManager: setStpPortState(): call MiscConfigService.setStpPortState() fail (null in result), nodeId = {}", nodeId);
                return false;
            }
    ...
    public StpPortState getStpPortState(Long nodeId, Short port){
        ...
        if (config == null) {
            config = this.session.getRpcService(MiscConfigService.class);
            if (config == null) {
                logger.debug("ERROR: ConfigManager: getStpPortState(): MiscConfigService is null, nodeId = {}", nodeId);
                return null;
            }
        }
        ...
        try {
            ...
            if(ret == null){
                logger.debug("ERROR: ConfigManager: getStpPortState(): call MiscConfigService.getStpPortState() fail (return null), nodeId {} port {}", nodeId, port);
                return null;
            }
            RpcResult<GetStpPortStateOutput> result = ret.get();
            if(result == null){
                logger.debug("ERROR: ConfigManager: getStpPortState(): call MiscConfigService.getStpPortState() fail (null result), nodeId {} port {}", nodeId, port);
                return null;
            }
            if(result.getResult() == null){
                logger.debug("ERROR: ConfigManager: getStpPortState(): call MiscConfigService.getStpPortState() fail (null in result), nodeId = {}", nodeId);
                return null;
            }
            StpPortState state = result.getResult().getStpPortState();
            if(state == null){
                logger.debug("ERROR: ConfigManager: getStpPortState(): call MiscConfigService.getStpPortState() with nodeId {} port {}, return null", nodeId, port);
                return null;
            }
            return state;
        } catch (InterruptedException ie) {
    ...
    public ArpEntry getArpEntry(Long nodeId, String ipAddress){
        ...
        if (config == null) {
            config = this.session.getRpcService(MiscConfigService.class);
            if (config == null) {
                logger.debug("ERROR: ConfigManager: getArpEntry(): MiscConfigService is null, nodeId = {}", nodeId);
                return null;
            }
        }
        ...
        try {
            ...
            if(ret == null){
                logger.debug("ERROR: ConfigManager: getArpEntry(): call MiscConfigService.getArpEntry() fail (return null), nodeId {} ipAddress {}", nodeId, ipAddress);
                return null;
            }
            RpcResult<GetArpEntryOutput> result = ret.get();
            if(result == null){
                logger.debug("ERROR: ConfigManager: getArpEntry(): call MiscConfigService.getArpEntry() fail (null result), nodeId {} ipAddress {}", nodeId, ipAddress);
                return null;
            }
            if(result.getResult() == null){
                logger.debug("ERROR: ConfigManager: getArpEntry(): call MiscConfigService.getArpEntry() fail (null in result), nodeId {} ipAddress {}", nodeId, ipAddress);
                return null;
            }
            ArpEntry entry = result.getResult();
            if(entry == null){
                logger.debug("ERROR: ConfigManager: getArpEntry(): call MiscConfigService.getArpEntry() with , nodeId {} ipAddress {}, return null", nodeId, ipAddress);
                return null;
            }
            return entry;
        } catch (InterruptedException ie) {
    ...
    public boolean setArpEntry(Long nodeId, String ipAddress, Long macAddress){
        ...
        if (config == null) {
            config = this.session.getRpcService(MiscConfigService.class);
            if (config == null) {
                logger.debug("ERROR: ConfigManager: setArpEntry(): MiscConfigService is null, nodeId = {}", nodeId);
                return false;
            }
        }
        ...
        try {
            ...
            if(result == null){
                logger.debug("ERROR: ConfigManager: setArpEntry(): call MiscConfigService.setArpEntry() fail (null result), nodeId {} ipAddress {} macAddress {}", nodeId, ipAddress, macAddress);
                return false;
            }
            if(result.getResult() == null){
                logger.debug("ERROR: ConfigManager: setArpEntry(): call MiscConfigService.setArpEntry() fail (null in result), nodeId {} ipAddress {} macAddress {}", nodeId, ipAddress, macAddress);
                return false;
            }
    ...
    public List<ArpTableEntry> getArpTable(Long nodeId){
        ...
        if (config == null) {
            config = this.session.getRpcService(MiscConfigService.class);
            if (config == null) {
                logger.debug("ERROR: ConfigManager: getArpTable(): MiscConfigService is null, nodeId = {}", nodeId);
                return null;
            }
        }
        ...
        try {
            ...
            if(ret == null){
                logger.debug("ERROR: ConfigManager: getArpTable(): call MiscConfigService.getArpTable() fail (return null), nodeId {}", nodeId);
                return null;
            }
            RpcResult<GetArpTableOutput> result = ret.get();
            if(result == null){
                logger.debug("ERROR: ConfigManager: getArpTable(): call MiscConfigService.getArpTable() fail (null result), nodeId {}", nodeId);
                return null;
            }
            if(result.getResult() == null){
                logger.debug("ERROR: ConfigManager: getArpTable(): call MiscConfigService.getArpTable() fail (null in result), nodeId {}", nodeId);
                return null;
            }
            List<ArpTableEntry> entryList = result.getResult().getArpTableEntry();
            if(entryList == null){
                logger.debug("ERROR: ConfigManager: getArpTable(): call MiscConfigService.getArpTable() with nodeId {}, fail", nodeId);
                return null;
            }
            return entryList;
        } catch (InterruptedException ie) {
            logger.debug("ERROR: ConfigManager: getArpTable(): call MiscConfigService.getArpTable() with nodeId {}, InterruptedException: {}", nodeId, ie);
            return null;
        } catch (ExecutionException ee) {
            logger.debug("ERROR: ConfigManager: getArpTable(): call MiscConfigService.getArpTable() with nodeId {}, ExecutionException: {}", nodeId, ee);
            return null;
        }
    ...
    public boolean deleteArpEntry(Long nodeId, String ipAddress){
        ...
        if (config == null) {
            config = this.session.getRpcService(MiscConfigService.class);
            if (config == null) {
                logger.debug("ERROR: ConfigManager: deleteArpEntry(): MiscConfigService is null, nodeId = {}", nodeId);
                return false;
            }
        }
        ...
        try {
            ...
            if(result == null){
                logger.debug("ERROR: ConfigManager: deleteArpEntry(): call MiscConfigService.deleteArpEntry() fail (null result), nodeId {} and ipAddress {}", nodeId, ipAddress);
                return false;
            }
            if(result.getResult() == null){
                logger.debug("ERROR: ConfigManager: deleteArpEntry(): call MiscConfigService.deleteArpEntry() fail (null in result), nodeId = {}", nodeId);
                return false;
            }
