        ...
        if (imp.equals(TopologyServices.class)) {
            ...
            c.add(createServiceDependency()
                    ...
                    .setCallbacks("setRefreshInternalProvider",
                            "unsetRefreshInternalProvider").setRequired(false));
            if(topo != null){
                topo.setMdNotifService();
            }
            else{
                logger.debug("ERROR: Activator: try to set TopologyServices.class to TopologyProvider topo, but topo is null! (this case should not happen, since topo is creatd in Activator's beginning)");
            }
        }
