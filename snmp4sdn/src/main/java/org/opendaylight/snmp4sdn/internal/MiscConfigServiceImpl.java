...
public class MiscConfigServiceImpl implements /*IConfigService,*/MiscConfigService/*md-sal*/, CommandProvider{
   ...
   public Status disableSourceLearning(Node node, NodeConnector nodeConnector){
    ...
    public Future<RpcResult<GetStpPortRootOutput>> getStpPortRoot(GetStpPortRootInput input){
        ...
        RpcResult<GetStpPortRootOutput> rpcResult =
                    Rpcs.<GetStpPortRootOutput> getRpcResult(true, ob.build(),
                            Collections.<RpcError> emptySet());
        return Futures.immediateFuture(rpcResult);
    }
    ...
    public Future<RpcResult<GetArpEntryOutput>> getArpEntry(GetArpEntryInput input){
        /*
        * Fix bug 5367: exception error of rest api get-arp-entry due to incorrect future value returned.
        */
    ...
    public Future<RpcResult<SetArpEntryOutput>> setArpEntry(SetArpEntryInput input){
        ...
        if(input == null){
            ...
            SetArpEntryOutputBuilder ob = new SetArpEntryOutputBuilder().setSetArpEntryResult(Result.FAIL);
            RpcResult<SetArpEntryOutput> rpcResult =
                    Rpcs.<SetArpEntryOutput> getRpcResult(false, ob.build(),
                            Collections.<RpcError> emptySet());
            return Futures.immediateFuture(rpcResult);
        }
        Long nodeId = input.getNodeId();
        ...
        if(status.isSuccess()){
            ...
            RpcResult<SetArpEntryOutput> rpcResult =
                    Rpcs.<SetArpEntryOutput> getRpcResult(true, ob.build(),
                            Collections.<RpcError> emptySet());
            return Futures.immediateFuture(rpcResult);
        }
        else{
