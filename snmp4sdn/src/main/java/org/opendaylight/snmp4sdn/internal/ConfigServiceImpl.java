...
System.out.println("2");System.out.println("2-2");
    ...
    //This is a temperary function for the CLI "_testCfgGetARPTable()" for debugging "gogo: InvocationTargetException: null" error in the implementation of MD-SAL API getArpTable()
    public List<ArpTableEntry> toGetArpTable(Long nodeId){
    ...
    //This is a temperary CLI for debugging "gogo: InvocationTargetException: null" error in the implementation of MD-SAL API getArpTable()
    //CLI: testCfgGetARPTable <switch>
    public void _testCfgGetARPTable(CommandInterpreter ci){
        String arg2 = ci.nextArgument();
        String garbage = ci.nextArgument();

        if(arg2 == null || garbage != null){
            ci.println();
            ci.println("Please use: testCfgGetARPTable <switch>");
            return;
        }
        ...
        List<ArpTableEntry> arpTable = toGetArpTable(nodeId);
        if(arpTable == null){
            ci.println();
            ci.println("Fail to get ARP Table on node " + nodeId);
            ci.println();
